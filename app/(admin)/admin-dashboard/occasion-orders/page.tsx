...
export default function OccasionOrders() {
    ...
    const fetchOrders = async () => {
      try {
        ...
        const filtered = res.data.data.filter((order: MenuOccasionOrderType) => 
          order.items.some((item: {orderType: string}) => item.orderType === "OCCASION-CAKES")
        );
        setOrders(filtered);
      } catch (error) {
  ...
  return (
    <div className="lg:p-5">
      <h1 className="text-2xl text-center font-semibold mb-4">Occasion Orders</h1>
